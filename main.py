...
def main():
    ...
    with open(config['options']['ctrl.yaml_file'], 'r') as yaml_file:
        yaml_config = yaml.safe_load(yaml_file)

    model_structure = 'resnet18'
    opts = AttrDict(yaml_config)
    opts.logger = logger
    ...
    # Ensure that the dataset name is correctly set
    if 'dataset' in opts and 'name' in opts['dataset']:
        dataset_name = opts['dataset']['name']
        opts.logger(f'Using dataset: {dataset_name}')

    im_size = opts['data']['im_size']
    augment = opts['data']['augment']

    # MODEL
    # NOTE: we use cpu mode for demo; change to gpu for experiments
    net = CTMNet(opts)
    model_structure = opts.model.structure
    net.set_model_structure(model_structure)
