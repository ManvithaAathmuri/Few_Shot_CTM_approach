...
class CTMNet(nn.Module):
    ...
    def __init__(self, opts, model_structure='shallow'):
        ...
        if model_structure == 'shallow':
            # Set up your model for the 'shallow' structure
            self.repnet = nn.Sequential(
                nn.Conv2d(3, 64, kernel_size=3, padding=1),
                nn.ReLU(),
                # Add more layers as needed
            )
        elif model_structure == 'resnet18':
            # Set up your model with the ResNet18 structure
            resnet = torchvision.models.resnet18(pretrained=True)
            # Remove the last layer (fully connected)
            self.repnet = nn.Sequential(*list(resnet.children())[:-1])
            # Add your own fully connected layer
            self.fc = nn.Linear(512, num_classes)
        else:
            raise ValueError("Invalid model_structure. Use 'shallow' or 'resnet18'.")
        ...
        if self.opts.fsl.ctm:
            ...
            if self.baseline_manner == 'sample_wise_similar':
                assert self.opts.model.structure == 'resnet18'
    ...
    """def set_model_structure(self, model_structure):
        ...
        # Define other layers and components based on the 'shallow' structure

        # For example, if you need to add more layers for 'shallow':
        # self.some_layer = nn.Sequential(
        ...
        # Set up your model with the ResNet18 structure
            self.repnet = torchvision.models.resnet18(pretrained=True)
        # Add your custom layers for the ResNet18 structure

        # For example, you can remove the fully connected layer and add your own:
            self.repnet = nn.Sequential(
                *list(self.repnet.children())[:-1],  # Remove the last layer (fully connected)
                nn.Linear(512, num_classes)  # Add your own fully connected layer
            )

        # Rest of the ResNet18-specific configuration...

        else:
        # Handle other model structures if needed
            pass"""
